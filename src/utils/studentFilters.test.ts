...
const makeStudent = (id: string, institutionName: string, teacherName: string, grade: string, className?: string): StudentItem => ({
  id, name: id, nickname: id, account: id, grade, className, school: institutionName, textbook: '人教版', institutionId: institutionName,
...
const students = [
  makeStudent('甲', '一中', '张老师', '初一', '一班'),
  makeStudent('乙', '一中', '李老师', '初二', '二班'),
  makeStudent('丙', '二中', '王老师', '高一'),
];

test('组织筛选选项按机构和老师逐级收窄', () => {
  assert.deepEqual(getStudentFilterOptions(students, {}), { institutions: ['一中', '二中'], teachers: ['张老师', '李老师', '王老师'], classes: ['一班', '二班'], grades: ['初一', '初二', '高一'] });
  ...
  assert.deepEqual(filterStudents(students, { className: '一班' }).map((item) => item.id), ['甲']);
});
