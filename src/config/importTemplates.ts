...
export const importTemplates: Record<ImportTemplateKey, ImportTemplateDefinition> = {
  ...
  questions: {
    fileName: '精选题库导入模板',
    headers: [...knowledgePointBaseFields.map((field) => field.label), '题型', '难度', '题干', '选项', '答案', '解析', '题干图片', '选项图片'],
    exampleRows: [['初中', '数学', '初一', '人教版', '数与代数', '一元一次方程', '方程应用', '单选题', '基础', '示例题干', 'A.1；B.2；C.3；D.4', 'A.1', '示例解析', '', '']],
  },
  knowledgePoints: {
    fileName: '知识点导入模板',
