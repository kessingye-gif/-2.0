...
test('平台总部导航围绕服务内容和用户使用组织', () => {
  ...
  assert.deepEqual(items.map((item) => item.label), ['运营首页', '服务产品', '内容管理', '机构管理', '学生管理', '系统管理']);
...
test('不同身份只看到被授权模块', () => {
  assert.deepEqual(getNavGroupsForRole('institution_admin').flatMap((group) => group.items).map((item) => item.id), ['dashboard', 'students', 'content']);
  assert.deepEqual(getNavGroupsForRole('teacher').flatMap((group) => group.items).map((item) => item.id), ['dashboard', 'students', 'content']);
  assert.equal(canAccessRoute('institution_admin', 'classes'), false);
  assert.equal(canAccessRoute('institution_admin', 'teachers'), false);
