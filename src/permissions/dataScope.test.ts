...
import { scopeInstitutions, scopeStudents, scopeTeachers } from './dataScope';
...
const students = [
  ...
  { id: 'S-2', institutionId: 'I-1', teacherId: 'T-2' },
...
test('教师只看到本人及本人负责的学生', () => {
  ...
  assert.deepEqual(scopeStudents(students, principal).map((item) => item.id), ['S-1']);
});
