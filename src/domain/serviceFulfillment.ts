...
export function createServiceFulfillment({ student, servicePackage, now, nonce, contentPackages = [], existingRights = [] }: CreateServiceFulfillmentInput): ServiceFulfillmentResult {
  ...
  const selectableContentPackageCount = Math.max(1, servicePackage.selectableContentPackageCount ?? 1);
  if (contentPackages.length > selectableContentPackageCount) throw new Error(`该服务包最多选择 ${selectableContentPackageCount} 个内容包`);
  ...
  if (latestActiveRight) {
    return {
      right: {
        ...
        todayAiUsage: latestActiveRight.todayAiUsage ?? 0,
