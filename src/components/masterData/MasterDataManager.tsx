...
const AdminAccountManager: React.FC<{ onCountChange: (count: number) => void }> = ({ onCountChange }) => {
  const [accounts, setAccounts] = useState<AdminAccount[]>(initialAdminAccounts);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState({ name: '', username: '', phone: '', password: generateRandomPassword() });
  const [error, setError] = useState('');
  useEffect(() => { onCountChange(accounts.length); }, [accounts.length, onCountChange]);
  const resetForm = () => { setEditingId(null); setForm({ name: '', username: '', phone: '', password: generateRandomPassword() }); setError(''); };
  const save = () => {
    ...
    const passwordMessage = form.password ? getPasswordValidationMessage(form.password) : '';
...
export const MasterDataManager: React.FC = () => {
  ...
  const [adminAccountCount, setAdminAccountCount] = useState(initialAdminAccounts.length);
